  ...
  const result = await db
    ...
    .where(
      and(
        ...
        parent === null ? isNull(folders.parentPath) : eq(folders.parentPath, parent),
...
foldersRoute.post('/', async (c) => {
  ...
  if (parentPath !== null) {
    const parentFolder = await db
      ...
      .from(folders)
      .where(
        and(eq(folders.userId, userId), eq(folders.path, parentPath), isNull(folders.deletedAt)),
      )
      .limit(1);
...
foldersRoute.patch('/:id', async (c) => {
  ...
  const body = await c.req.json<{ name?: string; parentPath?: string | null }>();
  ...
  if (body.parentPath !== undefined && body.parentPath !== null) {
    if (!body.parentPath.startsWith('/')) {
      return c.json({ error: 'parentPath must start with /' }, 400);
    }
  ...
  if (newParentPath !== null) {
    const parentFolder = await db
      ...
      .from(folders)
      .where(
        and(eq(folders.userId, userId), eq(folders.path, newParentPath), isNull(folders.deletedAt)),
      )
      .limit(1);
...
foldersRoute.patch('/:id/position', async (c) => {
  const userId = c.var.user.id;
  const folderId = c.req.param('id');
