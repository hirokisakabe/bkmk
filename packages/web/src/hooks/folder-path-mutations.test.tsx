...
function controlledFolderPatches(responses: ({ data: Folder } | { error: string })[]) {
  const controls = responses.map(() => {
    ...
    return {
      ...
      waitForResolution: new Promise<void>((resolve) => {
  ...
  server.use(
    http.patch('/api/folders/:id', async () => {
      ...
      if (!control || !response) {
        return HttpResponse.json({ error: 'unexpected folder patch' }, { status: 500 });
      }

      control.markStarted();
      await control.waitForResolution;
      return 'data' in response
        ? HttpResponse.json(response.data)
        : HttpResponse.json({ error: response.error }, { status: 500 });
    }),
  );

  return { controls, requestCount: () => requestCount };
}
...
describe('folder path mutation hooks', () => {
  ...
  it('pending 中に開始した rename の失敗が成功済み move のcacheを巻き戻さない', async () => {
    const beforeFolders = [
      makeFolder({ id: 'archive', name: 'archive', path: '/archive', parentPath: null }),
      makeFolder({ id: 'work', name: 'work', path: '/work', parentPath: null }),
    ];
    ...
    const patches = controlledFolderPatches([{ data: movedFolder }, { error: 'rename failed' }]);
    const { queryClient, Wrapper } = createWrapper();
    queryClient.setQueryData(foldersKey, beforeFolders);
    queryClient.setQueryData(bookmarksKey('/work'), sourceBookmarks);
    ...
    const { result } = renderHook(
      () => ({
        move: useMoveFolder({ selectedFolder: null, onSelectFolder: vi.fn() }),
        rename: useRenameFolder({ selectedFolder: null, onSelectFolder: vi.fn() }),
      }),
      { wrapper: Wrapper },
    );
    let moveMutation!: Promise<Folder>;
    act(() => {
      moveMutation = result.current.move.mutateAsync({ id: 'work', parentPath: '/archive' });
    });
    await patches.controls[0].started;

    let renameMutation!: Promise<Folder>;
    act(() => {
      renameMutation = result.current.rename.mutateAsync({ id: 'work', name: 'job' });
    });

    expect(patches.requestCount()).toBe(1);
    expect(queryClient.getQueryData(bookmarksKey('/work'))).toEqual(sourceBookmarks);
    expect(queryClient.getQueryData<Bookmark[]>(bookmarksKey('/archive/work'))).toEqual([
      expect.objectContaining({ id: 'source', folderPath: '/archive/work' }),
    ]);
    ...
    await act(async () => {
      patches.controls[0].resolvePatch();
      await moveMutation;
    });
    await patches.controls[1].started;

    expect(queryClient.getQueryData(bookmarksKey('/work'))).toBeUndefined();
    expect(queryClient.getQueryData<Bookmark[]>(bookmarksKey('/archive/job'))).toEqual([
      expect.objectContaining({ id: 'source', folderPath: '/archive/job' }),
    ]);

    await act(async () => {
      patches.controls[1].resolvePatch();
      await expect(renameMutation).rejects.toThrow('rename failed');
    });

    expect(queryClient.getQueryData(bookmarksKey('/work'))).toBeUndefined();
    expect(queryClient.getQueryData(bookmarksKey('/archive/job'))).toBeUndefined();
    expect(queryClient.getQueryData<Bookmark[]>(bookmarksKey('/archive/work'))).toEqual([
      expect.objectContaining({ id: 'source', folderPath: '/archive/work' }),
    ]);
    expect(queryClient.getQueryData<Folder[]>(foldersKey)).toEqual([
